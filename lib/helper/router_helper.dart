...
import 'package:flutter_restaurant/view/screens/auth/login_otp_screen.dart';
...
//*******Handlers*********
  ...
  static Handler _categoryHandler = Handler(handlerFunc: (context, Map<String, dynamic> params) {
    return CategoryScreen(categoryModel: CategoryModel(
      id: int.parse(params['id'][0]), image: params['image'][0], name: params['name'][0].replaceAll('-', ' '),
    ));
  });
  ...
  static Handler _mapHandler = Handler(handlerFunc: (context, Map<String, dynamic> params) {
    return MapWidget(address: AddressModel(
      id: int.parse(params['id'][0]), userId: int.parse(params['user'][0]), contactPersonName: params['name'][0], contactPersonNumber: params['num'][0],
      latitude: params['lat'][0], longitude: params['long'][0], addressType: params['type'][0], address: params['address'][0],
    ));
  });
  ...
  static Handler _selectLocationHandler = Handler(handlerFunc: (context, Map<String, dynamic> params) => SelectLocationScreen());
  ...
  static Handler _otpHandler = Handler(handlerFunc: (context, Map<String, dynamic> params){
    SignUpModel signUpModel = SignUpModel(
      fName: params['firstName'][0],
      lName: params['lastName'][0],
      email: params['email'][0],
      // password: params['password'][0],
      phone: params['number'][0],
    );
   return OtpScreen(signUpModel:signUpModel);
  });

  static Handler _loginOtpHandler = Handler(handlerFunc: (context, Map<String, dynamic> params){
    return LoginOtpScreen(phone:params['number'][0]);
  });
...
//*******Route Define*********
  static void setupRouter() {
    ...
    router.define(Routes.OTP_SCREEN, handler: _otpHandler, transitionType: TransitionType.fadeIn);
    router.define(Routes.LOGIN_OTP_SCREEN, handler: _loginOtpHandler, transitionType: TransitionType.fadeIn);
  }
}