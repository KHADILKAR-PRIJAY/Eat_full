          ...
          child: Column(
            ...
            children: [
              ...
              VxCard(Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  "${getTranslated('enter_the_otp_sent_to',context)} ${widget.signUpModel.phone}".text
                      .bold
                      .size(14)
  ...
  void _initFirebase(BuildContext context) async{
    ...
    if (!kIsWeb) {
      await _auth.verifyPhoneNumber(
            phoneNumber: "+"+widget.signUpModel.phone,
      ...
      {
        confirmationResult = await _auth.signInWithPhoneNumber("+"+widget.signUpModel.phone,
            RecaptchaVerifier(
          container: 'recaptcha',
    ...
    if (kIsWeb) {
      confirmationResult.confirm(otpCtl.text).then((value) {
        ...
        authProvider.registration(
            widget.signUpModel).then((status) async {
          if (status.isSuccess) {
          ...
          showCustomSnackBar(authProvider.registrationErrorMessage, context);
